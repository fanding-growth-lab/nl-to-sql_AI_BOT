...
계획을 바로 작성하지 말고, 먼저 추론 과정을 거친 후 계획을 작성하세요.
...
4. **데이터 정합성** - 생성/변환된 데이터가 의도대로 올바르게 처리되었는가? (결과값 확인)
   - **중요**: 조회나 계산 결과가 'Empty DataFrame'이거나 비어있는 경우, 이것이 정상적인 상황인지(데이터가 없는 것이 맞는 경우) 아니면 오류인지(쿼리 조건 실수, 데이터 로드 실패 등) 판단하세요.
   - 데이터가 있어야 정상인 상황에서 비어있다면 **반드시 `is_valid: false`**로 판정하고 원인을 분석하세요.
...
STATE = {
    ...
    "rag_schema_context": (
        "## t_member_push_device\n\n"
        "**설명:** [모바일 회원이 로그인한 기기 정보, 로그인 시간을 저장하는 테이블]\n\n"
        "| 컬럼명 | 데이터 타입 | 설명 |\n"
        "| --- | --- | --- |\n"
        "| `no` | `int(10) unsigned` | [로그인 로그를 고유하게 식별하는 번호 (PK)] |\n"
        "| `member_no` | `int(10) unsigned` | [해당 회원의 member_no. ( 즉, 크리에이터도 멤버의 한 종류)] |\n"
        "| `device` | `varchar(20)` | [로그인한 기기 정보 (apple, android)] |\n"
        "| `ins_datetime` | `datetime` | [로그인 시간] |\n"
        "---\n\n"
        "## t_creator\n\n"
        "**설명:** [크리에이터 정보 테이블]\n\n"
        "| 컬럼명 | 데이터 타입 | 설명 |\n"
        ...
        "| `del_datetime` | `datetime` | [크리에이터 서비스 탈퇴일] |\n"
        "| `is_active` | `char(1)` | [크리에이터 활성화 여부(T/F)] |\n"
        "---\n\n"
        "## t_creator_coupon\n\n"
        "**설명:** [크리에이터가 발급한 쿠폰에 대한 테이블]\n\n"
        "| 컬럼명 | 데이터 타입 | 설명 |\n"
        "| --- | --- | --- |\n"
        "| `no` | `int(10) unsigned` | [쿠폰 고유 식별 번호 (PK)] |\n"
        "| `creator_no` | `int(10) unsigned` | [해당 쿠폰을 발행한 크리에이터의 번호 (FK)] |\n"
        "| `name` | `varchar(50)` | [쿠폰 이름] |\n"
        "| `type` | `varchar(17)` | [쿠폰 타입 (discount, subscription)] |\n"
        "| `code` | `varchar(17)` | [쿠폰 활성화 코드] |\n"
        "| `duration` | `int(10) unsigned` | [쿠폰 혜택 기간 (일)] |\n"
        ...
        "| `attendance` | `char(1)` | [출석 여부 (T/F)] |\n"
        "| `status` | `char(1)` | [상태 (W: 대기, T: 완료, F: 실패)] |\n"
        "| `attend_datetime` | `datetime` | [출석 날짜] |\n"
        "| `ins_datetime` | `datetime` | [신청 로그 생성 날짜] |\n"
        "---\n\n"
        "## t_fanding\n\n"
        "**설명:** [크리에이터 - 회원 간 멤버십 관계 테이블]\n\n"
        "| 컬럼명 | 데이터 타입 | 설명 |\n"
        "| --- | --- | --- |\n"
        ...
        "| `item` | `varchar(20)` | [결제 상품 구분 (F: 멤버십, E: 이벤트, V: 온라인 강의)] |\n"
        ...
        "| `status` | `char(1)` | [결제 상태 ('T'= 결제완료,'P'=부분결제완료)] |\n"
        ...
        "| `status` | `varchar(10)` | [발행 상태 - public : 발행, private : 미발행, draft : 퍼블리싱 전] |\n"
        "| `public_range` | `char(1)` | [공개 범위 (A:전체,F:회원,C:유료,T:멤버십 지정)] |\n"
        "| `content_type` | `char(1)` | [컨텐츠 유형 (M : 동영상, I : 이미지, A: 오디오)] |\n"
        ...
        "| `public_status` | `varchar(10)` | [공개 상태 (public | partial | private)] |\n"
        "| `is_renewable` | `char(1)` | [갱신 가능 여부 (T/F)] |\n"
        "| `end_criteria` | `varchar(10)` | [종료 기준 : period(정해진 기간) | reward(정해진 횟수) | date(is_renewable = 'F'일 경우만 가능)] |\n"
        ...
        "| `is_private` | `char(1)` | [비공개 여부 (T/F)] |\n"
        "| `is_approval_required` | `char(1)` | [승인 필요 여부 (T/F)] |\n"
        "| `is_monthly_pass_allowed` | `char(1)` | [월간패스 허용 여부 (T/F)] |\n"
