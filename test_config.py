...
- **날짜 조건 처리**:
  - **신규/이탈/갱신 판단이 필요한 경우 (`t_fanding` 등)**: 시작일 기준 **1개월 전**부터 데이터를 조회합니다. (예: 3월 분석 시 2월 데이터부터 필요)
  - **단순 집계/로그 조회 (`t_payment`, `t_post_view_log` 등)**: 사용자 요청 기간과 **정확히 일치**하게 조회합니다.
- `비즈니스 규칙`을 따른다.
...
4. (날짜 조건 검증) 쿼리 목적에 맞는 올바른 날짜 범위를 사용했는가? (반드시 예)
   ...
   - **일반 조회 쿼리** (포스트 조회 로그, 매출 등): 분석 대상 기간만 조회
   - **[예외 허용]**: 만약 'Block Logic' 분석을 위해 전체적으로 1개월 여유를 둔 경우, 일반 쿼리(`t_payment` 등)에도 동일한 여유 기간이 적용되었다면 이를 **오류로 처리하지 말고 허용**하세요. (데이터 일관성을 위한 조치로 간주)
...
**🧩 Step 2: 스키마 검증 (매우 중요)**
- 사용할 각 테이블에 대해, 필요한 컬럼이 실제로 `관련 테이블 정보`에 존재하는지 확인하세요.
- **주의**: `ins_datetime`이나 `is_active` 같은 컬럼이 모든 테이블에 있다고 가정하지 마세요. 반드시 스키마를 확인하세요.
- 만약 필요한 컬럼이 없다면, 대체 컬럼을 찾거나 해당 단계를 수정하세요.

**🧩 Step 3: 단계 분해 (Decomposition)**
...
□ **[필수] 사용하려는 모든 컬럼이 제공된 `relative_tables` 스키마에 존재하는지 확인했습니까?** (존재하지 않는 컬럼 사용 시 실패 처리됨)

**출력 형식** (JSON 객체):

```json
{{
    "reasoning": "1. 목표 분석: ... \n2. 스키마 확인: t_fanding_log에는 ins_datetime이 없고 start_date가 있으므로 이를 사용... \n3. 단계 설계: ...",
    "plan": [
        "1. 조회 크리에이터 정보 (nickname으로 creator_no 매칭)",
        "2. 조회 Payment 데이터 (remain_price 포함)",
        "3. 변환 날짜 컬럼 (pay_datetime)",
        "4. 집계 월별 매출액",
        "5. 출력 최종 결과"
    ]
}}
...
PROMPT_REFINE_QUERY = """
사용자 질문을 분석하여 더 명확하고 실행 가능한 형태로 정제합니다.
...
**정제 규칙**:
1. **날짜 구체화**: "오늘", "이번 달", "작년" 등의 상대적 날짜 표현을 `{current_date}` 기준으로 구체적인 날짜(YYYY-MM-DD)로 변환하세요.
2. **용어 정규화**: 약어, 은어, 사내 용어를 표준 용어로 변경하세요.
   - "프페", "프페셀", "프페셸", "프로페셔널" -> "professional"
3. **분석 목적 명확화**: 질문이 모호하다면, 데이터 분석 관점에서 구체적인 목적을 드러내도록 문장을 다듬으세요.
   - 예: "A와 B 월 성과를 비교해줘" -> "A 크리에이터와 B 크리에이터의 이번 달 매출액 및 멤버십 가입자 수를 비교해줘"
4. **크리에이터 명확화**: 크리에이터를 명확하게 구분하세요.
   - 예: 강환국 작가의 월 매출 -> "강환국 작가"의 월 매출

**출력 형식**:
정제된 질문 텍스트만 출력하세요. (따옴표, 주석 없이)
"""
...
**🚫 [Critical] Mock Data 및 Mock Class 사용 절대 금지 🚫**
1. **절대 임의의 데이터를 생성하지 마세요.**
   - `data = {{'name': ['A', 'B'], 'value': [10, 20]}}` (X)
   - `df = pd.DataFrame(...)` (X)
2. **절대 Mock Class를 정의하지 마세요.**
   - `class MockEngine: ...` (X)
   - `class MockConnection: ...` (X)
   - `engine = MockEngine()` (X)
3. **반드시 `python_context`에 있는 변수나 DB 조회를 통해 데이터를 얻으세요.**
   - `engine` 변수는 이미 컨텍스트에 로드되어 있습니다. 새로 생성하지 말고 그대로 사용하세요.
   - `df = pd.read_sql(query, engine)` (O)
4. 데이터가 없다면 차라리 에러를 발생시키거나 빈 DataFrame을 반환하세요. 가짜 데이터를 만들면 즉시 실패로 간주합니다.
5. **변수 정의 필수**: `target_year`, `target_month` 등 코드 내에서 사용하는 모든 변수는 반드시 정의되어야 합니다. (예: `target_year = 2025`, `target_month = 10` 또는 `datetime` 객체에서 추출)
6. **데이터 검증**: `df.empty` 체크를 수행하고, 데이터가 없을 경우 적절한 메시지를 출력하거나 0으로 처리하는 로직을 포함하세요.
...
1.2 필요 데이터 파악
- Q: 필요한 데이터/변수는?
- Q: python_context에 이미 존재하는가? (`engine` 변수 확인)
...
**📍 Step 3: 코드 작성 원칙**
1. **[중요] `engine` 변수는 이미 존재하므로 새로 생성하지 마세요.**
...
**[중요] 피드백 작성 가이드**:
- **탐색적 제안 금지**: "컬럼을 확인해보세요", "데이터를 출력해보세요"와 같은 제안은 하지 마세요. 에이전트는 대화형으로 디버깅할 수 없습니다.
- **구체적 해결책 제시**: 에러가 발생했다면, **즉시 수정할 수 있는 구체적인 방법**을 제시하세요.
  - (X) "컬럼명이 맞는지 확인하세요."
  - (O) "'fanding_ins_datetime' 컬럼은 존재하지 않습니다. 'ins_datetime' 또는 'start_date'를 사용하세요."
- **코드 수정 지시**: 다음 번 코드 생성 시 바로 반영할 수 있도록 명확한 지시를 내리세요.

출력 형식:
```json
{{"is_valid": <true 또는 false>, "feedback": <true인 경우 빈 문자열, false인 경우 위 가이드에 따른 구체적인 수정 지시>}}
```
"""
...
STATE = {
    ...
    "rag_schema_context": (
        ...
        "| 컬럼명 | 데이터 타입 | 설명 |\n"
        "| --- | --- | --- |\n"
        "| `no` | `int(11) unsigned` | [**PK**] 팬딩(멤버십 가입 건) 고유 식별 번호. (다른 테이블에서는 `fanding_no`로 참조됨) |\n"
