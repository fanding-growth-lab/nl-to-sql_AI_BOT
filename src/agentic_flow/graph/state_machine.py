...
def create_agent_graph(
    db_schema: Optional[Dict[str, Any]] = None, config: Optional[Dict[str, Any]] = None
) -> Any:
    """
    ...
    graph.add_node(
        ...
        lambda state: nodes["llm_intent_classifier"].process(state),
    )
    graph.add_node(
        "nlp_processing", lambda state: nodes["nlp_processor"].process(state)
    )
    graph.add_node(
        "schema_mapping", lambda state: nodes["schema_mapper"].process(state)
    )
    graph.add_node(
        "dynamic_sql_generation",
        lambda state: nodes["dynamic_sql_generator"].process(state),
    )
    graph.add_node(
        "sql_generation", lambda state: nodes["sql_generator"].process(state)
    )
    graph.add_node(
        "sql_validation", lambda state: nodes["sql_validator"].process(state)
    )
    graph.add_node(
        "validation_check", lambda state: nodes["validation_node"].process(state)
    )
    graph.add_node(
        "user_review", lambda state: nodes["user_review_node"].process(state)
    )
    graph.add_node("sql_execution", _execute_sql_query)
    graph.add_node(
        "data_summarization", lambda state: nodes["data_summarizer"].process(state)
    )
...
def route_after_validation(state: AgentState) -> str:
    ...
    if state.get("conversation_response", False):
        logger.info(
            "Conversation response detected, skipping validation and proceeding to execution"
        )
    ...
    if retry_count < max_retries:
        logger.warning(
            f"SQL validation failed, retrying ({retry_count + 1}/{max_retries})"
        )
        # 재시도 카운트 증가
        ...
        logger.warning(
            "Max retries exceeded, proceeding to validation check for final decision"
        )
...
def route_after_validation_check(state: AgentState) -> str:
    ...
    if (
        ...
        and validation_result.get("confidence", 0) >= 0.85
    ):
    ...
    if isinstance(validation_result, dict):
        ...
        if status_value and (
            hasattr(status_value, "value")
            and status_value.value == "rejected"
            or status_value == "rejected"
        ):
            logger.warning(
                "Query rejected due to validation issues, requiring user review"
            )
            return "user_review"
...
def _execute_sql_query(state: AgentState) -> AgentState:
    ...
    try:
        ...
        logger.info(
            f"Query executed successfully in {execution_time:.2f}s, returned {len(query_result)} rows"
        )
