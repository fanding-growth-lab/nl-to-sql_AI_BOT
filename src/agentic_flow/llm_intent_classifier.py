...
class LLMIntentClassifier(BaseNode):
    ...
    def _initialize_llm(self) -> Optional[ChatGoogleGenerativeAI]:
        ...
        try:
            ...
            self.logger.info(
                f"Intent LLM config: model={settings.llm.intent_model}, "
            ...
            return ChatGoogleGenerativeAI(
                ...
                api_key=(
                    SecretStr(settings.llm.api_key) if settings.llm.api_key else None
                ),
            ...
            self.logger.warning(
                f"Failed to initialize LLM for intent classification: {str(e)}"
            )
            return None
...
{{
    ...
    def process(self, state: GraphState) -> GraphState:
        ...
        try:
            ...
            if not self.llm:
                self.logger.warning(
                    "LLM not available, skipping LLM intent classification"
                )
                state["llm_intent_result"] = None
                is_error = True
                return state
            ...
            if classification_result:
                self.logger.info(
                    f"LLM Intent Classification: {classification_result.intent.value} (confidence: {classification_result.confidence:.2f})"
                )
            ...
            if user_query:
                self._record_classification_stats(
                    user_query, classification_result, start_time, is_error
                )
    ...
    def _record_classification_stats(
        self,
        user_query: str,
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Intent classification stats integration failed: {e}"
                )
